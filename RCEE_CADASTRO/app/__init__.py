...
def create_app(config_name="default"):
    ...
    def home():
        ...
        # 🔑 Query base:
        # - Admin vê todos os posts (publicados + rascunhos de qualquer um)
        # - Demais usuários veem:
        #     - publicações publicadas
        #     - rascunhos que eles mesmos criaram
        query = Post.query
        if hasattr(current_user, "role") and current_user.role == "Admin":
            # Admin: sem filtro de status aqui (vê tudo)
            pass
        else:
            query = query.filter(
                or_(
        ...
        # Ordenar:
        # - Publicados por published_at desc
        # - Rascunhos (que muitas vezes não têm published_at) caem para created_at
        posts = query.order_by(  
            case((Post.published_at.is_(None), 1), else_=0),  # NULLs vão pro final  
