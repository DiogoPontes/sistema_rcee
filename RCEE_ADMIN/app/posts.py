...
KAFKA_TOPIC_ADMIN_TO_CONSULTA = os.getenv("KAFKA_TOPIC_ADMIN_TO_CONSULTA", "post_approved")
...
def publish_post_approved(post):
    """
    Publica evento completo do post aprovado para o CONSULTA via Kafka.
    """
    try:
        ...
        producer.produce(
            KAFKA_TOPIC_ADMIN_TO_CONSULTA,
            key=str(post.id),
            value=json.dumps(evento),
            callback=_delivery_report
        )
        producer.flush(timeout=10.0)
        logger.info("Evento post_approved publicado para post %s no tópico %s", post.id, KAFKA_TOPIC_ADMIN_TO_CONSULTA)
        return True
    except Exception as e:
        logger.error("Falha ao publicar post_approved para post %s: %s", post.id, e)
        return False
...
def edit(post_id):
    ...
    if form.validate_on_submit():
        ...
        # Sincroniza com o CADASTRO via Kafka
        try:
            producer = get_kafka_producer()
            evento = {
                ...
                "featured_image": post.featured_image,
                "category_id": post.category_id,
                "status": novo_status.name if hasattr(novo_status, "name") else str(novo_status),
            ...
            logger.info("Evento full_sync enviado: post %s -> %s", post.id, novo_status)
        except Exception as e:
            logger.error("Falha ao sincronizar com CADASTRO: %s", e)

        # Sincroniza com o CONSULTA via Kafka
        status_anterior_str = status_anterior.name if hasattr(status_anterior, "name") else str(status_anterior)
        novo_status_str = novo_status.name if hasattr(novo_status, "name") else str(novo_status)

        if novo_status_str.lower() == "approved":
            logger.info("Post %s aprovado. Publicando no CONSULTA...", post.id)
            publish_post_approved(post)
        else:
            logger.info("Post %s mudou para '%s'. Notificando CONSULTA para remover...", post.id, novo_status_str)
            try:
                producer = get_kafka_producer()
                evento_consulta = {
                    ...
                    "id": post.id,
                    "post_id": post.id,
                    "status": novo_status_str.lower(),
                }
                producer.produce(
                    KAFKA_TOPIC_ADMIN_TO_CONSULTA,
                    key=str(post.id),
                    value=json.dumps(evento_consulta),
                    callback=_delivery_report
                )
                producer.flush(timeout=5.0)
                logger.info("Evento de remoção enviado ao CONSULTA para post %s", post.id)
            except Exception as e:
                logger.error("Falha ao notificar CONSULTA sobre remoção do post %s: %s", post.id, e)
...
def add_upload(post_id):
    ...
    asset_type = AssetType.PDF if ext.lower() == ".pdf" else (
        AssetType.IMAGE if ext.lower() in [".png", ".jpg", ".jpeg", ".gif"] else AssetType.FILE
    )
    asset = PostAsset(
        post_id=post.id,
        asset_type=asset_type,
